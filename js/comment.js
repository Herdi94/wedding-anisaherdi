...
export const comment = (() => {
    ...
    const reply = (button) => {
        ...
        <div class="d-flex flex-wrap justify-content-end align-items-center mb-0">
            <button style="font-size: 0.8rem;" onclick="comment.cancel('${id}')" class="btn btn-sm btn-light bg-red color-theme-white rounded-3 py-0 me-1">Cancel</button>
            <button style="font-size: 0.8rem;" onclick="comment.send(this)" data-uuid="${id}" class="btn btn-sm btn-light bg-blue color-theme-white rounded-3 py-0">Send</button>
        </div>`;
    ...
    const edit = async (button) => {
        ...
        if (status?.code === 200) {
            ...
            <div class="d-flex flex-wrap justify-content-end align-items-center mb-0">
                <button style="font-size: 0.8rem;" onclick="comment.cancel('${id}')" class="btn btn-sm btn-light bg-red color-theme-white rounded-3 py-0 me-1">Cancel</button>
                <button style="font-size: 0.8rem;" onclick="comment.update(this)" data-uuid="${id}" class="btn btn-sm btn-light bg-blue color-theme-white rounded-3 py-0">Update</button>
            </div>`;
        ...
        await request(HTTP_GET, `/api/comment?per=${pagination.getPer()}&next=${pagination.getNext()}`)
            ...
            .then((res) => {
                ...
                if (res.data.length === 0) {
                    comments.innerHTML = `<div class="h6 text-center fw-bold p-4 my-3 bg-pink color-theme-white rounded-4 shadow">Yuk bagikan undangan ini biar banyak komentarnya</div>`;
                    return;
                }
